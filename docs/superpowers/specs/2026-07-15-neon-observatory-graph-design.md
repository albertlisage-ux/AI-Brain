...
- 星云光晕以 12 秒周期缓慢漂移，扫描线以 18 秒周期匀速移动；两者都不接收鼠标事件。
- 当前激活的图谱面板使用 8 秒周期的低亮度边缘呼吸光，不改变节点位置或分组颜色。
- 在“减少动态效果”模式下完全关闭装饰动画和过渡，并恢复静态背景。
...
- CSS 括号配对，无网络字体、高频动画或作用于编辑器的选择器。
- CSS 包含 12 秒星云漂移、18 秒扫描线移动、8 秒激活面板呼吸光，以及 `prefers-reduced-motion` 静态回退。
- 重载 Obsidian 后，全局图谱呈现深蓝背景、网格、动态扫描线和霓虹主题节点。
